                ...
                <div class="layui-card-body ">
                    <form class="layui-form layui-col-space5">
                        <div class="layui-inline layui-show-xs-block">
                            <input type="text" name="name" placeholder="分类名" autocomplete="off"
                                   class="layui-input">
                        </div>
                        <div class="layui-inline layui-show-xs-block">
...
<script type="text/html" id="nameTemp">
    <button type="button" class="layui-btn layui-btn-sm {{d.info}}">{{d.name}}</button>
</script>
...
<script>
    ...
    layui.use(['table', 'form'], function () {
        ...
        table.render({
            ...
            url: "tag/page",
            cols: [[
                {title: "编号", field: "tagId"},
                {title: "标签名", field: "name", templet: "#nameTemp"},
                {title: "简介", field: "profile"},
        ...
        table.on("tool(dataList)", function (obj) {
            const {data: {tagId}, event} = obj;
            if (event === "delete") {
                layer.confirm("确定删除编号[" + tagId + "]的数据吗?", {icon: 3}, function (index) {
                    $.ajax({
                ...
                xadmin.open("修改标签", "tag/edit?tagId=" + tagId)
            }
        });
        table.on("toolbar(dataList)", function (obj) {
            if (obj.event === "add") {
                xadmin.open("添加标签", "tag/edit")
            }
        });
