                ...
                <div class="layui-card-body ">
                    <form class="layui-form layui-col-space5">
                        <div class="layui-inline layui-show-xs-block">
                            <input type="text" name="name" placeholder="评论人" autocomplete="off"
                                   class="layui-input">
                        </div>
                        <div class="layui-inline layui-show-xs-block">
...
<script>
    ...
    layui.use(['table', 'form'], function () {
        ...
        table.render({
            ...
            url: "comment/page",
            cols: [[
                {title: "编号", field: "commentId"},
                {title: "评论人", field: "name"},
                {title: "评论内容", field: "content"},
                {title: "评论博客", field: "title"},
        ...
        table.on("tool(dataList)", function (obj) {
            const {data: {commentId}, event} = obj;
            if (event === "delete") {
                layer.confirm("确定删除编号[" + commentId + "]的数据吗?", {icon: 3}, function (index) {
                    $.ajax({
                ...
                layer.confirm("确定更改编号[" + commentId + "]的状态吗?", {icon: 3}, function (index) {
                    $.ajax({
                        url: "comment/status/",
                        data: {
                            commentId,
